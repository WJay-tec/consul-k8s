	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			commonHelmValues := map[string]string{
				...
				"global.image":    "hashicorp/consul-enterprise:1.13.0-alpha1-ent",
				"global.imageK8S": "docker.mirror.hashicorp.services/hashicorpdev/consul-k8s-control-plane:latest",
			...
			// Copy secret from dc1 to dc2
			copySecret(t, staticClientPeerClusterContext, staticServerPeerClusterContext, "api-token")
			// Create the peering dialer
			...
			logger.Logf(t, "creating namespaces %s in acceptor peer", staticServerNamespace)
			...
			logger.Logf(t, "creating namespaces %s in dialer peer", staticClientNamespace)
			...
			// ie default -> blue and blue -> default.
			...
			helpers.Cleanup(t, cfg.NoCleanupOnFailure, func() {
				_, err := staticServerPeerClient.ConfigEntries().Delete(api.ServiceIntentions, "*", &api.WriteOptions{})
				require.NoError(t, err)
			})
			...
			logger.Log(t, "creating intentions in acceptor peer")
